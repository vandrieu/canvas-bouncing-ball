...
import { MouseEventHandler, MouseState } from "./types";

const DUMMY_MOUSE_TOUCH_ID = 9999

export function setupMouseListeners(mouseState: MouseState, canvas: HTMLCanvasElement, onUp: MouseEventHandler, onDown: MouseEventHandler, onMove: MouseEventHandler) {
  ...
  canvas.addEventListener('mousedown', (e) => {
    if (e?.buttons === 1) { //left click only
      ...
      onDown(DUMMY_MOUSE_TOUCH_ID, new Victor(e.x, e.y))
    }
  })
  canvas.addEventListener('touchstart', (e) => {
    ...
    [...e.changedTouches].forEach(touch => {
      ...
      onDown(touch.identifier, new Victor(touch.pageX, touch.pageY))
    })
  })
  ...
  canvas.addEventListener('mousemove', (e) => {
    ...
    onMove(DUMMY_MOUSE_TOUCH_ID, new Victor(e.x, e.y))
  })
  canvas.addEventListener('touchmove', (e) => {
    [...e.changedTouches].forEach(touch => {
      updateMouseState_onMove(mouseState, touch.identifier, touch.pageX, touch.pageY)
      onMove(touch.identifier, new Victor(touch.pageX, touch.pageY))
    })
  })
  ...
  canvas.addEventListener('mouseup', (e) => {
    if (e?.buttons === 0) { //left click only
      ...
      onUp(DUMMY_MOUSE_TOUCH_ID, new Victor(e.x, e.y))
    }
  })
  canvas.addEventListener('touchend', (e) => {
    [...e.changedTouches].forEach(touch => {
      updateMouseState_onUp(mouseState, touch.identifier, touch.pageX, touch.pageY)
      onUp(touch.identifier, new Victor(touch.pageX, touch.pageY))
    })
  })
}
...
function updateMouseState_onMove(mouseState: MouseState, touchId: number, x: number, y: number) {
  mouseState.pos[touchId] = new Victor(x, y)
}
...
function updateMouseState_onDown(mouseState: MouseState, touchId: number, x: number, y: number) {
  ...
  mouseState.down[touchId] = new Victor(x, y)
}
